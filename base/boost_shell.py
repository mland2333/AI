...
df = pd.read_csv(path_shell, header=0)
X = df.iloc[:, :-3]
...
# 划分训练集和测试集
X_train, X_test, y_train, y_test = train_test_split(X_scaled, y_encoded, test_size=0.2, random_state=46)
model = xgb.XGBClassifier()# 定义模型
# 超参数调整
param_grid = {'max_depth': [3, 5, 7,8], 'learning_rate': [0.1,0.2, 0.3, 0.5], 'n_estimators': [50,70, 80, 100]}
